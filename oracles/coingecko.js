...
export class CoinGecko {
	...
	_updateOracleData(data, blockNumber) {
		...
		if (susdsPrice.gt(this._lastPrices[this._SUSDS])) {
			...
			console.log(`INFO(sUSDS): coinGecko price is lower, last_price=${this._lastPrices[this._SUSDS].toFixed(this._precission)}, oracle_price=${susdsPrice.toFixed(8)}`);
		}
		...
		if (susdePrice.gt(this._lastPrices[this._SUSDE])) {
			...
			console.log(`INFO(sUSDe): coinGecko price is lower, last_price=${this._lastPrices[this._SUSDE].toFixed(this._precission)}, oracle_price=${susdePrice.toFixed(8)}`);
		}
