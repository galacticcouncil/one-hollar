...
export class Strategy {
	...
	async findOpportunities() {
		...
		for (const [k, v] of Object.entries(this.#config)) {
			paris.push({id: k, assets: [ this.#hollar, v.assetId]})
		}
		...
		for (const price of prices) {
			...
			console.log(`INFO(${p.id}): price=${price.val.toFixed(5)}[${price.id}/H], target_usd_price=${priceUSD.toFixed(5)}[$/${price.id}], target_price=${targetPrice.toFixed(5)}[${price.id}/H], buy_at_limit=${buyAt.toFixed(5)}[${price.id}/H], sell_at_limit=${sellAt.toFixed(5)}[${price.id}/H]`);
